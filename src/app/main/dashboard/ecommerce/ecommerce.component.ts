...
export class EcommerceComponent implements OnInit {
  ...
  getTotalUsers() {
    this._dashboardService.onApiDataChanged.subscribe(response => {
      this.data.totalUser = response;
    });
  }

  getTodaysRegisteredUser() {
    this.userService.getRegisteredToday(1,10,'').subscribe(res=>{
      this.data.registeredUser = res.total ;
      console.log('total registeredUser',this.data.registeredUser);
    })
  }

  getOnlineUsers() {
    this.userService.getOnlineUsers(1,10,'').subscribe(res=>{
      this.data.count = res.total ;
      console.log('total Online',this.data.count);
    })
  }
